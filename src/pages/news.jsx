				...
				{allContentfulArticle.edges.map(({ node: article }) => (
					<li key={article.id} className={styles.article}>
						<div className={styles.articleImage}>
							{article.image && (
								<Link to={`/news/${article.slug}`}>
									<Image
										media={article.image}
										alt={article.image.description || article.image.title}
									/>
								</Link>
							)}
						</div>
						<div className={styles.articleInfo}>
							...
							<div>{article.intro ? renderRichText(article.intro) : null}</div>
						</div>
					</li>
				))}
			</ul>
...
export const Head = () => (
	<Seo title='News' description='Listing of our news articles' />
);
